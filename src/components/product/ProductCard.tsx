...
export function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Product Image */}
        <Link href={{ pathname: "/urunler/[handle]", params: { handle: product.handle } }} className="block relative">
        ...
        <div className="flex flex-col flex-1 p-5 gap-4">
          <div>
            ...
            <Link href={{ pathname: "/urunler/[handle]", params: { handle: product.handle } }}>
          ...
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 bg-[#F5F3EE] border border-[#EEECE6] text-[#4A4A4A] text-xs font-medium px-3 py-1.5 rounded-full">
              ~{product.portions} {t("per_portion")}
            </span>
            {product.hasPump && (
              <span className="text-xs" style={{ color: "#8A8A7A" }}>{t("pump_dosing")}</span>
            )}
          </div>
